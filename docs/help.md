...
### Where Caja Scripts Are Stored

Caja scripts are usually placed in

[~/.config/caja/scripts/](~/.config/caja/scripts/)

To use them, ensure they are executable (e.g., chmod +x scriptname). Once added, they appear under the "Scripts" option in Caja’s right-click menu.

Useful Caja Scripts for Better Functionality, here is an example.

1. **Hide Files Without Renaming**

   - **Purpose**: Hides selected files without adding a dot (.) prefix, using Caja’s .hidden file feature.

   - **Source**: Inspired by Muna Bedan’s work on GitHub (munabedan/Caja-Scripts).

   - **Script Example**:

     ```bash
     #!/bin/bash
     # Hide selected files by adding them to .hidden
     for file in "$CAJA_SCRIPT_SELECTED_FILE_PATHS"; do
         basename "$(echo "$file")" >> "$(dirname "$file")/.hidden"
     done
     ```
